...
def update_sheet
  ...
  consultation_list = Consultation.joins('LEFT OUTER JOIN patient_sources on patient_sources.consultation_id=consultations.id').order('consultations.id, patient_sources.created_at').joins('LEFT JOIN patients on consultations.patient_id=patients.id').joins('LEFT OUTER JOIN coupons on consultations.coupon_id=coupons.id').order('consultations.id asc').pluck('DISTINCT on (consultations.id) consultations.id, consultations.patient_id, patients.name, patients.mobile, coupons.coupon_code, consultations.category, consultations.user_status, consultations.pay_status, consultations.created_at, patient_sources.local_referrer, patient_sources.utm_campaign')
